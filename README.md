...
<img width="1194" height="896" alt="Banana Boxing" src="https://github.com/user-attachments/assets/13d89c62-f1cd-4f81-beb4-bd89904c5888" />
...
2. **Нажмите «Подключить»** — появится окно поиска Bluetooth. Выберите устройство **FruitNinja**.
3. **Подождите 2–3 секунды** после подключения — контроллер автоматически калибруется (держите его неподвижно).
4. **Выберите тип удара** из четырёх карточек:
   - 👊➡️ **Джеб правой**
   - ⬅️👊 **Джеб левой**
   - 🪝➡️ **Хук правой**
   - ⬅️🪝 **Хук левой**
5. **Выполняйте удар** — система автоматически фиксирует движение при превышении порога. Счётчик на карточке увеличивается.
...
6. **Повторите** для остальных трёх типов удара.
7. **Нажмите «Обучить»** — начнётся обучение (100 эпох, ~1–2 минуты). Прогресс виден на экране.
8. После завершения можно **протестировать** — система начнёт распознавать удары в реальном времени.
9. **Нажмите «Экспорт модели»** — скачается файл `gesture_model_tf.json`.
